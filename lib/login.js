...
module.exports = async function (url, username, password, options = {}) {
  ...
  for (let key in options.fields) {
    // console.log(key, '->', options.fields[key])
    if (!loginCredentials[key]) loginCredentials[key] = options.fields[key]
  }

  // temp !
  loginCredentials.__EVENTVALIDATION = data.__EVENTVALIDATION
  loginCredentials.__VIEWSTATE = data.__VIEWSTATE
  ...
  if (options.test) { let testRes = await performTest(options.test) }
...
async function performTest (options) {
  ...
  console.log(' > LOGIN CHECK: ', testResult, '===', options.shouldEqual, '?', '->', testPass)
  return testPass
}
